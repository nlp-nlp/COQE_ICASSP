...
from utils.functions import formulate_gold, formulate_gold_absa
...
class Trainer_absa(nn.Module):
    ...
    def train_model(self):
        ...
        if self.args.kind.lower() == 'coqe':
            ...
            self.create_path(path)
        elif self.args.kind.lower() == 'sub-to-absa':
            path = os.path.join(self.args.output_path, 'ckpt-sub-to-absa')
            self.create_path(path)
        ...
        for epoch in range(self.args.max_epoch):
            ...
            if f1 > best_f1:
                ...
                if self.args.kind.lower() == 'coqe':
                    ...
                    torch.save(self.model.state_dict(), open(os.path.join(self.args.output_path, 'ckpt-sub-to-absa', 'best.pt'), 'wb'))
                elif  self.args.kind.lower() == 'tuple-logits':
        ...
        if self.args.kind.lower() == 'coqe':
            ...
            self.model.load_state_dict(torch.load(open(os.path.join(self.args.output_path, 'ckpt-sub-to-absa', 'best.pt'), 'rb')))
            file_name = "sub_to_absa_pred_evaluation"
        elif self.args.kind.lower() == 'tuple-logits':
