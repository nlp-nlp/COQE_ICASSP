...
def process_line(args, text_line, label_line, tokenizer: AutoTokenizer, sample_id):
    ...
    for tri in raw_labels:
        ...
        aspect_offset = proc_raw_offset(tri[2], text, args.data_after_path)
        view_offset = proc_raw_offset(tri[3], text, args.data_after_path)

        sentiment_label = have_triples * EMO_MAP[int(tri[4])]

        if 'Camera' in args.data_after_path:
...
def load_data_absa(args, mode: str):
    raw_data = []
    with open(os.path.join(args.data_after_path, f'{mode}.txt'), 'r') as f:
        for line in f:
